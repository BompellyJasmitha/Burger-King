import React from "react";
import "../style/navBar.css";

class TopNavbar extends React.Component{
    
    render(){
        return(
            <div id="navbardiv">

                <a href="#" id="iconMenu"><i class="fa-solid fa-bars"></i></a>

                <span><img src="assets/images/Navbar title.png" alt="" id="bktitle" /></span>
                
                <a href="#"  id="iconPerson"><i class="fa-solid fa-user"></i></a>
                <button><i class="fa-solid fa-bag-shopping"></i> view Cart</button>
                 
            </div>

)}}



class BottomNavbar extends React.Component{    
    render(){
        return(
            <div id="BottomNavbar">
            
            <div><img src="data:image/png;base64," alt=""/><h5>Home</h5></div>
            
            <div><img src="https://www.bk.com/static/media/4f5f9366a38571baec86febd55f0769bc348f9d7-22x22.928445c72424e3a06f12.png" alt="" /><h5>Menu</h5></div>

            <div><img src="data:image/png;base64," alt="" /> <h5>Offers</h5></div>

            <div><img src="https://www.bk.com/static/media/daf5c681f2f8aafb1000074e9fa60c327055d59d-22x15.8999aa31fe345e72cf99.png" alt="" /><h5>Rewards</h5></div>
            
            </div>

)}}

export {TopNavbar,BottomNavbar}
